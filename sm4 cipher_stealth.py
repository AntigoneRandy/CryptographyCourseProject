...
# 电子密码本(ECB)
def encrypt_ecb_classic(plain_text, key):
    """
    SM4(ECB)加密
    :param plain_text: 明文
    ...
    plain_text = _padding(plain_text, mode=SM4_ENCRYPT) #plain_text bytes
    if plain_text is None:
        return

    # 密钥检验
    key = _key_iv_check(key_iv=key) #key bytes

    plain_hex = _hex(plain_text) #plain hex bytes
    cipher_hex_list = [] #list
    for i in _range(len(plain_text) // BLOCK_BYTE):
        sub_hex = plain_hex[i * BLOCK_HEX:(i + 1) * BLOCK_HEX] #subhex bytes
        cipher = encrypt(clear_num=int(sub_hex, 16), #cipher int
                         mk=int(_hex(key), 16))
        cipher_hex_list.append(num2hex(num=cipher, width=BLOCK_HEX))

    cipher_text = b64encode(_unhex(''.join(cipher_hex_list))) #cipher test bytes
    end = time.perf_counter()
    return cipher_text if PY2 else cipher_text, (end - start)*1000 # .decode(E_FMT)


def decrypt_ecb_classic(cipher_text, key):
    """
    SM4(ECB)解密
...
# 密码块链接(CBC)
def encrypt_cbc_classic(plain_text, key, iv):
    """
    SM4(CBC)加密
    :param plain_text: 明文
    :param key: 密钥, 小于等于16字节
    ...
    if plain_text is None: #plain test byte
        return

    # 密钥检验
    key = _key_iv_check(key_iv=key)#key byte
    # 初始化向量监测
    iv = _key_iv_check(key_iv=iv)#iv byte
...
def decrypt_cbc_classic(cipher_text, key, iv):
    """
    SM4(CBC)解密
    :param cipher_text: 密文
    :param key: 密钥 小于等于16字节
    ...
    return plain_text if PY2 else plain_text, (end - start) * 1000  # .decode(E_FMT)

# 电子密码本(ECB)
def encrypt_ecb_stealth(plain_text, key):
    """
    SM4(ECB)加密
    :param plain_text: 明文
    :param key: 密钥, 小于等于16字节
...
def decrypt_ecb_stealth(cipher_text, key):
    """
    SM4(ECB)解密
    :param cipher_text: 密文
    :param key: 密钥, 小于等于16字节
    ...
    block_num = len(cipher_text) // BLOCK_BYTE 
    if len(cipher_text) % BLOCK_BYTE != 0 : block_num += 1
    length_n_2 = len(cipher_hex[ (block_num-2)*BLOCK_HEX : -32 ])

    #decrypt n-2 blocks
    for i in _range(block_num - 2):
        sub_hex = cipher_hex[i * BLOCK_HEX:(i + 1) * BLOCK_HEX]
        plain = decrypt(cipher_num=int(sub_hex, 16),
                        mk=int(_hex(key), 16))
        plain_hex_list.append(num2hex(num=plain, width=BLOCK_HEX))
    ...
    #decrypt last block
    sub_hex = cipher_hex[ -32 :  ]
    plain_last = decrypt(cipher_num=int(sub_hex, 16),
                    mk=int(_hex(key), 16))
    plain_last = num2hex (num=plain_last , width=BLOCK_HEX ) #[0:length_n_2]

    #decrypt block n-1
    
    sub_hex = cipher_hex[ (block_num-2)*BLOCK_HEX : -32 ] + bytes(plain_last[ -(32-length_n_2) : ] , encoding='utf-8') 
    plain_n_2 = decrypt(cipher_num=int(sub_hex, 16),
                        mk=int(_hex(key), 16))
    plain_n_2 = num2hex (num=plain_n_2 , width=BLOCK_HEX )

    #write back
    plain_last = plain_last[0 : length_n_2]
    plain_hex_list.append(plain_n_2)
    plain_hex_list.append(plain_last)

    plain_text = _unhex(''.join(plain_hex_list))
    end = time.perf_counter()
    return plain_text if PY2 else plain_text, (end - start)*1000
    # .decode(E_FMT)
...
# 密码块链接(CBC)
def encrypt_cbc_stealth(plain_text, key, iv):
    """
    SM4(CBC)加密
    :param plain_text: 明文
    :param key: 密钥, 小于等于16字节
    ...
    cipher_temp = []
    for c in ivs[1:] :
        cipher_temp.append( num2hex(num=c, width=BLOCK_HEX))
    ...
    cipher_text = b64encode(_unhex( cipher_text)) 
        
    end = time.perf_counter()
    return cipher_text if PY2 else cipher_text, (end - start) * 1000  # .decode(E_FMT)


def decrypt_cbc_stealth(cipher_text, key, iv):
    """
    SM4(CBC)解密
    :param cipher_text: 密文
    :param key: 密钥 小于等于16字节
    ...
    short_length = len(cipher_hex) % BLOCK_HEX #byte
    #decrypt 1 ~ n-2 block
    for i in _range( block_num - 2 ):
        sub_hex = cipher_hex[i * BLOCK_HEX:(i + 1) * BLOCK_HEX]
        cipher = int(sub_hex, 16)
    ...
    #decrypt last block
    sub_hex = cipher_hex[-32 : ]
    n_1_hex = cipher_hex[ (block_num-2)*BLOCK_HEX : -32 ]
    cipher = int(sub_hex, 16)
    cipher_n_1 = int( n_1_hex,16 )
    plain = ((cipher_n_1 * pow(16,32-short_length)) ^ decrypt(cipher_num=cipher,
                                  mk=int(_hex(key), 16)))
    plain_last=(num2hex(num=plain, width=BLOCK_HEX))[0 : short_length]

    #decrypt n-1 block

    sub_hex = n_1_hex + num2hex(plain,width=16)[-(32-short_length) : ].encode(encoding=E_FMT)

    cipher = int(sub_hex, 16)
    plain = (ivs[-1] ^ decrypt(cipher_num=cipher,
                                  mk=int(_hex(key), 16)))
    plain_hex_list.append(num2hex(num=plain, width=BLOCK_HEX))
    plain_hex_list.append(plain_last)
...
def encrypt_ecb(plain_text , key ,mode):
    if mode == 'classic' :
        return encrypt_ecb_classic(plain_text,key)
    elif len(plain_text) % BLOCK_BYTE == 0 :
        return encrypt_ecb_classic(plain_text,key)
    else :
        return encrypt_ecb_stealth(plain_text,key)

def decrypt_ecb(cipher_text , key ,mode):
    if mode == 'classic' :
        return decrypt_ecb_classic(cipher_text,key)
    elif len(cipher_text) % BLOCK_BYTE == 0 :
        return decrypt_ecb_classic(cipher_text,key)
    else :
        return decrypt_ecb_stealth(cipher_text,key)

def encrypt_cbc(plain_text , key ,iv,mode):
    if mode == 'classic' :
        return encrypt_cbc_classic(plain_text,key,iv)
    elif len(plain_text) % BLOCK_BYTE == 0 :
        return encrypt_cbc_classic(plain_text,key,iv)
    else :
        return encrypt_cbc_stealth(plain_text,key,iv)

def decrypt_cbc(cipher_text , key , iv ,mode):
    if mode == 'classic' :
        return decrypt_cbc_classic(cipher_text,key,iv)
    elif len(cipher_text) % BLOCK_BYTE == 0 :
        return decrypt_cbc_classic(cipher_text,key,iv)
    else :
        return decrypt_cbc_stealth(cipher_text,key,iv)

def main() :
    text = '0000000000000000'
    keytest = '123456'
    ivtest = '11111111'
    cipher_text=encrypt_cbc(text,keytest,ivtest,'stealth')[0]
    print(decrypt_cbc(cipher_text, keytest,ivtest,'stealth')) 
